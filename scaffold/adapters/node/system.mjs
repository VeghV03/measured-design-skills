...
// discipline.
//
// This is a second reference adapter, proving the build contract is stack-agnostic:
// what is fixed is boards/<id>.html plus the ten model files, not this file, and not
// its sibling in ../python/system.py, which this deliberately mirrors token for token.
...
export function icon(name) {
  ...
  return `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden="true" `
    + `stroke="currentColor" stroke-width="1.5"><path d="${d}"/></svg>`;
}
...
export function emptyState(glyph, heading, line, action, foot = '') {
  return `<div class="empty" data-component="empty_state">${icon(glyph)}`
...
export function door(consequence, willNot, precondition, action) {
  const items = willNot.map(w => `<li>${w}</li>`).join('');
  return `<div data-component="door"><h1>${consequence}</h1>`
    + `<p>What this will not do:</p><ul>${items}</ul>`
    + `<p>${precondition}</p><button class="btn danger">${action}</button></div>`;
}

export function whoCell(who) {
  return `<span class="who" data-component="who_cell">${who}</span>`;
}

export function row(name, who, meta) {
  return `<div class="grid" data-component="row"><span>${icon('file')} ${name}</span>`
    + `${whoCell(who)}<span class="chip">${meta}</span></div>`;
}
